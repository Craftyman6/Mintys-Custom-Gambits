...
namespace Gambonanza.RoadWorkAheadsGambit
{
    ...
    /// The actual gameplay logic is in GambitRoadWorkAhead.cs.
    /// </summary>
    public sealed class RoadWorkAheadsGambitMod : IMod
    {
        public void OnLoad(IModContext context)
        {
            context.LogLine("[RoadWorkAheadsGambit] registering Road Work Ahead's Gambit.");

            // Optional custom art: put `RoadWorkAhead.png` next to mod.json.
            // In source form that means:
            //   RoadWorkAheadsGambit/RoadWorkAhead.png
            // After building/installing, sample_mods/build.sh copies it beside the DLL:
            //   Mods/RoadWorkAheadsGambit/RoadWorkAhead.png
            // If the file is missing, we generate a tiny placeholder so the mod still works.
            var spritePath = Path.Combine(context.ModDirectory, "RoadWorkAhead.png");
            ...
            // `give gambit RoadWorkAhead`, so keep it short and readable.
            var def = GambitBuilder.Create("RoadWorkAhead")
                .WithName("Road Work Ahead's Gambit")
                .WithDescription("Capturing an enemy piece <shake>crumbles</shake> the tile ahead of it.")
                .WithRarity(Rarity.RARE)
                .WithFocus(Gambit_Focus.CRUMBLE)
                .WithPrice(4)
                .WithVisual(sprite)
                .WithVisualScale(0.85f)
                // This tells GambitApi to attach GambitRoadWorkAhead to the in-run
                // gambit object. Without this, the card would exist but do nothing.
                .WithBaseGambit<GambitRoadWorkAhead>()
                // AutoUnlock means the gambit can appear immediately without adding
                // a separate unlock achievement.
                .AutoUnlock(true)
                .Register();

            context.LogLine($"[RoadWorkAheadsGambit] registered '{def.Id}'.");
        }
