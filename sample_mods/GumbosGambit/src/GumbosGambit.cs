...
namespace Gambonanza.GumbosGambit
{
    ...
    /// The actual gameplay logic is in GambitGumbo.cs.
    /// </summary>
    public sealed class GumbosGambitMod : IMod
    {
        public void OnLoad(IModContext context)
        {
            context.LogLine("[GumbosGambit] registering Gumbo's Gambit.");

            // Optional custom art: put `Gumbo.png` next to mod.json.
            // In source form that means:
            //   GumbosGambit/Gumbo.png
            // After building/installing, sample_mods/build.sh copies it beside the DLL:
            //   Mods/GumbosGambit/Gumbo.png
            // If the file is missing, we generate a tiny placeholder so the mod still works.
            var spritePath = Path.Combine(context.ModDirectory, "Gumbo.png");
            ...
            // `give gambit Gumbo`, so keep it short and readable.
            var def = GambitBuilder.Create("Gumbo")
                .WithName("Gumbo's Gambit")
                .WithDescription("When a tile falls, the tile under it begins to <shake>crumble</shake>.")
                .WithRarity(Rarity.COMMON)
                .WithFocus(Gambit_Focus.CRUMBLE)
                .WithPrice(4)
                .WithVisual(sprite)
                .WithVisualScale(0.85f)
                // This tells GambitApi to attach GambitGumbo to the in-run
                // gambit object. Without this, the card would exist but do nothing.
                .WithBaseGambit<GambitGumbo>()
                // AutoUnlock means the gambit can appear immediately without adding
                // a separate unlock achievement.
                .AutoUnlock(true)
                .Register();

            context.LogLine($"[GumbosGambit] registered '{def.Id}'.");
        }
